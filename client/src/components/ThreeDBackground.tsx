...
export default function ThreeDBackground({ scrollY }: ThreeDBackgroundProps) {
  ...
  useEffect(() => {
    ...
    const initParticles = () => {
      for (let i = 0; i < particleCount; i++) {
        particles.push({
          ...
          color: ['#00d9ff', '#ff006e', '#39ff14'][Math.floor(Math.random() * 3)],
          life: 1,
        });
      }
    };
    ...
    const drawTorus = (ctx: CanvasRenderingContext2D, shape: Shape) => {
      const majorRadius = shape.size;
      const minorRadius = shape.size * 0.3;
      const segments = 20;
      const rings = 20;
    ...
    const animate = () => {
      ...
      shapes.forEach((shape) => {
        ...
        shape.y += (scrollY * 0.1 - shape.y) * 0.05;
